var galTheme = {
	...
	renderHeader(state, instance) {
		...
		if(state.comments) {
			if(state.comments.length > 0) {
				...
				$(container).append('<i class="fa fa-comments"></i> No comment yet')
			}
		}
		return container
	},
	renderComments(state, instance) {
		...
		if (error) {
			...
			if (error.message === 'Comments Not Initialized'
				...
				initButton.innerText = 'Initialize the comment system'
		...
		if (state.comments) {
			for(let i = 0; i < state.comments.length; i++) {
				...
				replylink.innerText = isLogin ? 'Reply' : 'Log in to reply'
	...
	renderEditor(state, instance) {
		...
		$container.append('<h3 class="comment-reply-title"><i class="fa fa-pencil"></i> Welcome Comment</h3>')
		if (state.user.login) {
			...
			$container.append(`<div class="gitment-editor-write-field">
													<textarea id="comment" placeholder="express your opinion." cols="45" rows="7"></textarea>
												 </div>`)
			$container.append(`<p class="form-submit">
													<a class="gitment-editor-submit">Comment</a>
												 </p>`)
			...
			submitButton.onclick = function () {
				if(!submitButton.hasAttribute('disabled')) {
					submitButton.innerText = 'Submitting review...'
...
function formatDate(date) {
	...
	return year + 'Year' + month + 'Month' + day + 'Day ' + timeZone + date.getHours() + ':' + minutes
}
